...
\begin{algorithmic}
\REQUIRE{$p \gets \text{list of planets}$}
\STATE{$relay \gets in\_range(0, p_{length})$}
\STATE{$connected \gets \text{queue of planets}$}
\STATE{$disconnected \gets p - p[relay]$}
\STATE{$\text{push } p[relay] \text{ to } connected$}
...
\STATE{$\text{add } p[i] \text{ to } edge_{connections}$}
\STATE{$\text{add } edge \text{ to } p[i]_{connections}$}
\STATE{$disconnected \gets disconnected - p[i]$}
\STATE{$\text{push } p[i] \text{ to } connected$}
\ENDFOR
\ENDWHILE
\end{algorithmic}
\end{algorithm}
