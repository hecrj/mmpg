...
generated \textbf{300 systems} as a game world.
...
The main symptom was that the \texttt{API} was notifying 200 kbits of events per second, when before deploying the bandwidth
needed was 10-12 kbps. The events that changed after the deployment were the \textbf{snapshots} (see \autoref{log_system}).
...
its data was static. Instead, \textbf{static data and dynamic data could be separated}.
...
The optimization reduced the CPU usage from 20\% to \textbf{2\%} consistently and the bandwidth from 200kbps to \textbf{20kbps}.
