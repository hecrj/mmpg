...
A library that implements a set of useful classes to communicate with an \texttt{API} and implement
  game viewers.
\item[Game logic]
It includes the game world definition and the rules of the game. It is built on top of the engine.
\item[Game viewer]
...
\href{https://opensource.org/licenses/MIT}{The MIT License}.
\section{Documents}
...
\href{http://creativecommons.org/licenses/by-nc-sa/4.0/legalcode.txt}{Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International}
license.
\clearpage
\part{Planning}
\chapter{Time plan}
...
A library that will allow game programmers to build games and wire them to the platform
...
transparently. Stakeholders can try the software easily while it is being developed and give feedback. In this case,
...
There are many programming languages that allow to create HTTP APIs easily, like Pyhton, \texttt{Ruby}, \texttt{Elixir}... However,
...
password are valid or not. A credential validator can be injected in the \texttt{API} easily. Hence, anyone can
...
color. Hence, \texttt{GitHub}-based \texttt{Identicons} \cite{identicons} were used to identify different players easily. The \texttt{Identicons} are
...
Firstly, the requirement for \texttt{MMPGs} games was established. Then, it was shown that there was not
...
Secondly, the development of the solution was planned. A time plan and a budget for the solution were shown.
